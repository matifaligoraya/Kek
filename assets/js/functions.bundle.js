...
(function (global, factory) {
	...
	if( typeof kekOptions !== 'undefined' ) {
		options = Object.assign({}, options, kekOptions);
	}
	...
	var Core = function() {
		return {
			...
			colorScheme: function() {
				...
				var bodyColorScheme = Core.cookie.get('__kek_body_color_scheme');
			...
			viewport: function() {
				...
				document.documentElement.style.setProperty('--kek-viewport-width', viewport.width);
				document.documentElement.style.setProperty('--kek-viewport-height', viewport.height);
				document.documentElement.style.setProperty('--kek-body-height', vars.elBody.clientHeight);
	...
	var Base = function() {
		return {
			...
			goToTop: function() {
				kek.GoToTop.init('#gotoTop');
			},

			stickFooterOnSmall: function() {
				kek.StickFooterOnSmall && kek.StickFooterOnSmall.init('#footer');
			},
			...
			headers: function() {
				...
				kek.Headers.init('#header');
			},

			menus: function() {
				kek.Menus.init('#header');
			},

			pageMenu: function() {
				kek.PageMenu && kek.PageMenu.init('#page-menu');
			},

			sliderDimensions: function() {
				kek.SliderDimensions && kek.SliderDimensions.init('.slider-element');
			},

			sliderMenuClass: function() {
				kek.SliderMenuClass && kek.SliderMenuClass.init('.transparent-header + .swiper_wrapper,.swiper_wrapper + .transparent-header,.transparent-header + .revslider-wrap,.revslider-wrap + .transparent-header');
			},

			topSearch: function() {
				kek.TopSearch.init('#top-search-trigger');
			},

			topCart: function() {
				kek.TopCart.init('#top-cart');
			},

			sidePanel: function() {
				kek.SidePanel && kek.SidePanel.init('#side-panel');
			},

			adaptiveColorScheme: function() {
				kek.AdaptiveColorScheme && kek.AdaptiveColorScheme.init('.adaptive-color-scheme');
			},

			portfolioAjax: function() {
				kek.PortfolioAjax && kek.PortfolioAjax.init('.portfolio-ajax');
			},

			cursor: function() {
				if( vars.customCursor ) {
					kek.Cursor && kek.Cursor.init('body');
				}
			},
	...
	var Modules = function() {
		return {
					...
					if( notExec ) {
						el.getAttributeNames().some( function(text) {
							if( text.includes('data-bs') ) {
								notExec = false;
								kek.Bootstrap && kek.Bootstrap.init('body');
								return true;
							}
						});
					}
			...
			resizeVideos: function(element) {
				kek.ResizeVideos && kek.ResizeVideos.init(element ? element : 'iframe[src*="youtube"],iframe[src*="vimeo"],iframe[src*="dailymotion"],iframe[src*="maps.google.com"],iframe[src*="google.com/maps"]');
			},

			pageTransition: function() {
				if( vars.pageTransition ) {
					kek.PageTransition && kek.PageTransition.init('body');
				}
			},

			lazyLoad: function(element) {
				kek.LazyLoad && kek.LazyLoad.init(element ? element : '.lazy:not(.lazy-loaded)');
			},

			dataClasses: function() {
				kek.DataClasses && kek.DataClasses.init('[data-class]');
			},

			dataHeights: function() {
				kek.DataHeights && kek.DataHeights.init('[data-height-xxl],[data-height-xl],[data-height-lg],[data-height-md],[data-height-sm],[data-height-xs]');
			},

			lightbox: function(element) {
				kek.Lightbox && kek.Lightbox.init(element ? element : '[data-lightbox]');
			},

			modal: function(element) {
				kek.Modal && kek.Modal.init(element ? element : '.modal-on-load');
			},

			animations: function(element) {
				kek.Animations && kek.Animations.init(element ? element : '[data-animate]');
			},

			hoverAnimations: function(element) {
				kek.HoverAnimations && kek.HoverAnimations.init(element ? element : '[data-hover-animate]');
			},

			gridInit: function(element) {
				kek.Grid && kek.Grid.init(element ? element : '.grid-container');
			},

			filterInit: function(element) {
				kek.Filter && kek.Filter.init(element ? element : '.grid-filter,.custom-filter');
			},

			canvasSlider: function(element) {
				kek.CanvasSlider && kek.CanvasSlider.init(element ? element : '.swiper_wrapper');
			},

			sliderParallax: function() {
				kek.SliderParallax && kek.SliderParallax.init('.slider-parallax');
			},

			flexSlider: function(element) {
				kek.FlexSlider && kek.FlexSlider.init(element ? element : '.fslider');
			},

			html5Video: function(element) {
				kek.FullVideo && kek.FullVideo.init(element ? element : '.video-wrap');
			},

			youtubeBgVideo: function(element) {
				kek.YoutubeBG && kek.YoutubeBG.init(element ? element : '.yt-bg-player');
			},

			toggle: function(element) {
				kek.Toggle && kek.Toggle.init(element ? element : '.toggle');
			},

			accordion: function(element) {
				kek.Accordion && kek.Accordion.init(element ? element : '.accordion');
			},

			counter: function(element) {
				kek.Counter && kek.Counter.init(element ? element : '.counter');
			},

			countdown: function(element) {
				kek.Countdown && kek.Countdown.init(element ? element : '.countdown');
			},

			gmap: function(element) {
				kek.GoogleMaps && kek.GoogleMaps.init(element ? element : '.gmap');
			},

			roundedSkills: function(element) {
				kek.RoundedSkills && kek.RoundedSkills.init(element ? element : '.rounded-skill');
			},

			progress: function(element) {
				kek.Progress && kek.Progress.init(element ? element : '.skill-progress');
			},

			twitterFeed: function(element) {
				kek.Twitter && kek.Twitter.init(element ? element : '.twitter-feed');
			},

			flickrFeed: function(element) {
				kek.Flickr && kek.Flickr.init(element ? element : '.flickr-feed');
			},

			instagram: function(element) {
				kek.Instagram && kek.Instagram.init(element ? element : '.instagram-photos');
			},

			// Dribbble Pending

			navTree: function(element) {
				kek.NavTree && kek.NavTree.init(element ? element : '.nav-tree');
			},

			carousel: function(element) {
				kek.Carousel && kek.Carousel.init(element ? element : '.carousel-widget');
			},

			masonryThumbs: function(element) {
				kek.MasonryThumbs && kek.MasonryThumbs.init(element ? element : '.masonry-thumbs');
			},

			notifications: function(element) {
				kek.Notifications && kek.Notifications.init(element ? element : false);
			},

			textRotator: function(element) {
				kek.TextRotator && kek.TextRotator.init(element ? element : '.text-rotater');
			},

			onePage: function(element) {
				kek.OnePage && kek.OnePage.init(element ? element : '[data-scrollto],.one-page-menu');
			},

			ajaxForm: function(element) {
				kek.AjaxForm && kek.AjaxForm.init(element ? element : '.form-widget');
			},

			subscribe: function(element) {
				kek.Subscribe && kek.Subscribe.init(element ? element : '.subscribe-widget');
			},

			conditional: function(element) {
				kek.Conditional && kek.Conditional.init(element ? element : '.form-group[data-condition],.form-group[data-conditions]');
			},

			shapeDivider: function(element) {
				kek.ShapeDivider && kek.ShapeDivider.init(element ? element : '.shape-divider');
			},

			stickySidebar: function(element) {
				kek.StickySidebar && kek.StickySidebar.init(element ? element : '.sticky-sidebar-wrap');
			},

			cookies: function(element) {
				kek.Cookies && kek.Cookies.init(element ? element : '.gdpr-settings,[data-cookies]');
			},

			quantity: function(element) {
				kek.Quantity && kek.Quantity.init(element ? element : '.quantity');
			},

			readmore: function(element) {
				kek.ReadMore && kek.ReadMore.init(element ? element : '[data-readmore]');
			},

			pricingSwitcher: function(element) {
				kek.PricingSwitcher && kek.PricingSwitcher.init(element ? element : '.pricing-tenure-switcher');
			},

			ajaxTrigger: function(element) {
				kek.AjaxTrigger && kek.AjaxTrigger.init(element ? element : '[data-ajax-loader]');
			},

			videoFacade: function(element) {
				kek.VideoFacade && kek.VideoFacade.init(element ? element : '.video-facade');
			},

			schemeToggle: function(element) {
				kek.SchemeToggle && kek.SchemeToggle.init(element ? element : '.body-scheme-toggle');
			},

			clipboardCopy: function(element) {
				kek.Clipboard && kek.Clipboard.init(element ? element : '.clipboard-copy');
			},

			codeHighlight: function(element) {
				kek.CodeHighlight && kek.CodeHighlight.init(element ? element : '.code-highlight');
			},

			tips: function() {
				if( vars.tips ) {
					kek.Tips && kek.Tips.init('body');
				}
			},

			textSplitter: function(element) {
				kek.TextSplitter && kek.TextSplitter.init(element ? element : '.text-splitter');
			},

			mediaActions: function(element) {
				kek.MediaActions && kek.MediaActions.init(element ? element : '.media-wrap');
			},

			viewportDetect: function(element) {
				kek.ViewportDetect && kek.ViewportDetect.init(element ? element : '.viewport-detect');
			},

			scrollDetect: function(element) {
				kek.ScrollDetect && kek.ScrollDetect.init(element ? element : '.scroll-detect');
			},

			fontSizer: function(element) {
				kek.FontSizer && kek.FontSizer.init(element ? element : '.font-sizer');
			},

			hover3D: function(element) {
				kek.Hover3D && kek.Hover3D.init(element ? element : '.hover-3d');
			},

			buttons: function(element) {
				kek.Buttons && kek.Buttons.init(element ? element : '.button-text-effect');
			},

			bsComponents: function(element) {
				kek.BSComponents && kek.BSComponents.init(element ? element : '[data-bs-toggle="tooltip"],[data-bs-toggle="popover"],[data-bs-toggle="tab"],[data-bs-toggle="pill"],.style-msg');
			}
		};
	}();
	...
	var DocumentOnResize = function() {
		return {
			init: function() {
				...
				Custom.onResize();
				Core.addEvent( window, 'kekResize' );
			}
		};
	}();
...
(function (global, factory) {
	typeof exports === 'object' && typeof module !== 'undefined' ? module.exports = factory() :
	typeof define === 'function' && define.amd ? define(factory) :
	( global = typeof globalThis !== 'undefined' ? globalThis : global || self, global.kek = factory() );
} (this, (function() {
	// USE STRICT
	"use strict";
	...
	return {
		...
		Menus: function() {
			...
			var _hover = function() {
				...
				var menuHoverDelay = getComputedStyle(__core.getVars.elHeader).getPropertyValue('--kek-primary-menu-submenu-display-speed') || 666;
			...
			var _fullWidth = function() {
				...
				if( __core.getVars.elHeader.classList.contains('floating-header') ) {
					var floatingHeaderPadding = getComputedStyle(document.querySelector('#header')).getPropertyValue('--kek-header-floating-padding');
					document.querySelectorAll('.mega-menu:not(.mega-menu-full):not(.mega-menu-small) .mega-menu-content').forEach( function(el) {
			...
			return {
				init: function(selector) {
					...
					if( __core.getVars.elHeader.classList.contains('no-sticky') ) {
						...
						headerHeight = getComputedStyle(__core.getVars.elHeader).getPropertyValue('--kek-header-height').split('px')[0];
					} else {
						headerHeight = getComputedStyle(__core.getVars.elHeader).getPropertyValue('--kek-header-height-shrink').split('px')[0];
					}

					pageMenu.style.setProperty("--kek-page-submenu-sticky-offset", headerHeight+'px');
		...
		Cursor: function() {
			return {
				init: function(selector) {
					...
					var cursor = document.querySelector('.kek-cursor');
					var cursorFollower = document.querySelector('.kek-cursor-follower');
					var cursorDot = document.querySelector('.kek-cursor-dot');
					...
					if( !cursor ) {
						cursor = addCursorEl('.kek-cursor', __core.getVars.elWrapper);
					}

					if( !cursorFollower ) {
						cursorFollower = addCursorEl('.kek-cursor-follower', cursor);
					}

					if( !cursorDot ) {
						cursorDot = addCursorEl('.kek-cursor-dot', cursor);
					}
					...
					document.querySelectorAll('a,button').forEach( function(el) {
						el.addEventListener('mouseenter', function() {
							cursor.classList.add('kek-cursor-action');
						});

						el.addEventListener('mouseleave', function() {
							cursor.classList.remove('kek-cursor-action');
						});
					});

					document.querySelectorAll('.cursor-disable').forEach( function(el) {
						el.addEventListener('mouseenter', function() {
							cursor.classList.add('kek-cursor-disabled');
						});

						el.addEventListener('mouseleave', function() {
							cursor.classList.remove('kek-cursor-disabled');
						});
					});
				}
			};
				...
				init: function(selector) {
					...
					if( elLoaderColor ) {
						if( elLoaderColor == 'theme' ) {
							elLoaderCSSVar = ' style="--kek-loader-color:var(--kek-themecolor);"';
						} else {
							elLoaderCSSVar = ' style="--kek-loader-color:'+elLoaderColor+';"';
						}
					}
					...
					if( elSpeedIn ) {
						__core.getVars.elWrapper.style.setProperty('--kek-animate-duration', Number(elSpeedIn)+'ms');
						if( elAnimIn == 'fadeIn' ) {
							pageTransition.style.setProperty('--kek-animate-duration', Number(elSpeedIn)+'ms');
						}
					}
					...
					var endPageTransition = function() {
						...
						var displayContent = function() {
							...
							setTimeout(function() {
								__core.getVars.elWrapper.style.removeProperty('--kek-animate-duration');
							}, 666);
						};
					...
					__core.isFuncTrue( function() {
						...
						selector.each( function(){
							var element = jQuery(this),
								...
								elTargetValue = '__kek_' + elTarget.split('#')[1],
				...
				init: function(selector) {
					...
					__core.isFuncTrue( function() {
						...
						selector.forEach( function(element) {
							...
							var kekSwiper = new Swiper( element.querySelector('.swiper-parent'), {
							...
							if( slideNumberCurrent ) {
								if( elLoop == true ) {
									slideNumberCurrent.innerHTML = kekSwiper.realIndex + 1;
								} else {
									slideNumberCurrent.innerHTML = kekSwiper.activeIndex + 1;
								}
							}
				...
				init: function(selector) {
					...
					__core.isFuncTrue( function() {
						...
						selector.forEach( function(element) {
							...
							elBar.style.setProperty( '--kek-progress-speed', elSpeed );
					...
					__core.isFuncTrue( function() {
						...
						if( jQuery(elTarget).length > 0 && elTrigger == 'self' ) {
							...
							jQuery(elTarget).get(0).addEventListener('hidden.bs.toast', function() {
								kek.Notifications.init( selector );
							});
						}
		...
		Cookies: function() {
			...
			return {
				init: function(selector) {
					...
					if( elPersist == 'true' ) {
						__core.cookie.set('__kek_cookies_accept', '');
						__core.cookie.remove('__kek_cookies_decline');
					}
					...
					if( cookieBar ) {
						...
						if( __core.cookie.get('__kek_cookies_accept') != '1' ) {
							setTimeout( function() {
					...
					if( gdprAcceptBtn.length > 0 ) {
						gdprAcceptBtn.forEach( function(btn){
							btn.onclick = function(e) {
								...
								__core.cookie.set('__kek_cookies_accept', '1', elExpire);
								__core.cookie.set('__kek_cookies_decline', '0', elExpire);
					...
					if( gdprDeclineBtn.length > 0 ) {
						gdprDeclineBtn.forEach( function(btn){
							btn.onclick = function(e) {
								...
								__core.cookie.set('__kek_cookies_accept', '0', elExpire);
								__core.cookie.set('__kek_cookies_decline', '1', elExpire);
					...
					var acceptCookies = __core.cookie.get('__kek_cookies_accept'),
						declineCookies = __core.cookie.get('__kek_cookies_decline'),
						cookiesAllowed = (acceptCookies || acceptCookies != '0') && (!declineCookies || declineCookies != '1');

					elSwitches?.forEach( function(el) {
						var elCookie = '__kek_gdpr_' + el.getAttribute('data-cookie-name'),
					...
					document.querySelectorAll('.gdpr-container')?.forEach( function(element) {
						var elCookie = '__kek_gdpr_' + element.getAttribute('data-cookie-name'),
			...
			var _toggle = function(element, sibling=false, action=false) {
				...
				if( __core.contains( bodyClassToggle, __core.getVars.elBody ) ) {
					...
					if( remember == "true" && action ) {
						localStorage.setItem('kekBodyColorScheme', 'dark');
					}
					...
					// Remove Storage
					if( remember == "true" && action ) {
						localStorage.removeItem('kekBodyColorScheme');
					}
			...
			return {
				init: function(selector) {
					__core.isFuncTrue( function() {
						...
						if( typeof kekTips === "undefined" || kekTips.length < 1 ) {
							return false;
						}
						...
						var randomIndex = Math.floor(Math.random() * kekTips.length);
						var randomTip = kekTips[randomIndex];

						var tipsEl = document.getElementById('kek-tips-element');

						if( !tipsEl ) {
							var tipsHTML = '<div class="position-fixed bottom-0 end-0 p-3 text-start" style="z-index: 699;"><div id="kek-tips-element" data-notify-trigger="custom" data-notify-target="#kek-tips-element" data-notify-timeout="7777" class="toast hide p-3" role="alert" aria-live="assertive" aria-atomic="true" style="--bs-toast-max-width:400px;--bs-toast-bg: rgba(var(--bs-body-bg-rgb),.925);"><div class="toast-header bg-transparent border-0 mb-0 align-items-center pb-1"><h5 id="kek-tips-element-title" class="me-auto fs-6 fw-semibold mb-0"></h5><button type="button" class="btn-close me-1" data-bs-dismiss="toast" aria-label="Close"></button></div><div id="kek-tips-element-content" class="toast-body small pt-1"></div><a href="#" id="kek-tips-element-disable" class="text-muted text-decoration-underline op-06 h-op-08 px-2 pb-2 ms-1 mt-1 d-inline-block" data-cookies="true" style="font-size:.75rem;text-underline-offset:3px;">Disable Random Tips</a></div></div>';

							__core.getVars.elWrapper.insertAdjacentHTML('beforeend', tipsHTML);
							tipsEl = document.getElementById('kek-tips-element');
						}

						var tipsTitle = document.getElementById('kek-tips-element-title');
						var tipsContent = document.getElementById('kek-tips-element-content');
						var tipsDisable = document.getElementById('kek-tips-element-disable');
						var tipsEnable = document.getElementById('kek-tips-element-enable');
						...
						if( tipsDisable ) {
							tipsDisable.onclick = function(e) {
								...
								var tipsToast = bootstrap.Toast.getOrCreateInstance(document.getElementById('kek-tips-element'));
								tipsToast.hide();

								__core.cookie.set('__kek_tips_cookies', 'hide', 1);
							};
						}
						...
						if( tipsEnable ) {
							tipsEnable.onclick = function(e) {
								...
								__core.cookie.remove('__kek_tips_cookies');
								window.location.reload();
							};
						}
						...
						(function(){
							...
							if( __core.cookie.get('__kek_tips_cookies' ) == 'hide' ) {
								enabled = false;
							}
		...
		TextSplitter: function() {
			...
			var _splitter = function(el, type = 'word') {
				...
				el.querySelectorAll('span')?.forEach( function(elem, index) {
					elem.style.setProperty('--kek-split-index', index + 1);
				});
			};
			...
			var _percent = function(params) {
				...
				var onScroll = function(params) {
					fastdom.measure( function(){
						...
						params.elem.style.setProperty('--kek-scroll-percent', percent);
						params.elem.style.setProperty('--kek-scroll-ratio', ratio);
						params.elem.style.setProperty('--kek-scroll-start', start);
						params.elem.style.setProperty('--kek-scroll-end', end);
					});
				};
						...
						var initParams = function(selector) {
							...
							selector.forEach( function(elem) {
								...
								if( includeWidth == 'true' || (elem.classList.contains('parallax') && elem.getAttribute('data-parallax-direction') == 'horizontal') ) {
									elem.style.setProperty('--kek-scroll-width', params.width);
								}

								if( includeHeight == 'true' || (elem.classList.contains('parallax') && elem.getAttribute('data-parallax-direction') != 'horizontal') ) {
									elem.style.setProperty('--kek-scroll-height', params.height);
								}

								if( includeOffset == 'true' ) {
									elem.style.setProperty('--kek-scroll-offset', params.offset);
								}

								if( !isNaN(parallaxRatio) ) {
									elem.style.setProperty('--kek-parallax-ratio', parallaxRatio);
								}
