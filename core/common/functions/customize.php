 ...
 *
 * @package KeK\Core\Customize\Functions
 * @author   KeK
 ...
 */
function kek_customize_get_all_config(WP_Customize_Manager $wp_customize = null)
{
    static $configs = null;

    if (null === $configs) {
        $filtered_configs = apply_filters('zoo/customizer/config', [], $wp_customize);
        if (file_exists(kek_DIR.'inc/customize/customize.php')) {
            $theme_configs = require kek_DIR.'inc/customize/customize.php';
 ...
 * @return  array
 */
function kek_customize_get_panel_setting($panel_id)
{
    $options = array_merge([
        'desktop' => '',
        'mobile'  => '',
    ], (array)kek_customize_get_setting($panel_id, 'all'));
 ...
 * @return  mixed
 */
function kek_customize_get_setting($name, $device = 'desktop', $key = '', $default_only = false)
{
    $get_value = $value = null;
    $page_id   = kek_get_current_page_id();
    $configs   = kek_customize_get_all_config();
 ...
 */
function kek_customize_get_attachment($value, $size = 'full')
{
    if (empty($value)) {
        return false;
    }
