 ...
 */
function kek_hex2rgba($hex, $opacity = 1)
{
    $hex = str_replace("#", "", $hex);
 ...
 * @return  string
 */
function kek_px2rem($var)
{
    $body_font = get_theme_mod('kek_typo_body_font');
    $base = apply_filters( 'kek_body_font_size', 16 );
 ...
 * @return  string
 */
function kek_generate_css_color($class, $var)
{
    if ($var != '') {
        $color = $class . '{';
        $color .= "color: " . $var . ";}";
...
function kek_generate_css_font_size($class, $var)
{
    $font = $class . '{';
    $font .= "font-size: " . kek_px2rem((int)str_replace(' ', 'px', $var)) . ";}";
    return $font;
}
 ...
 * @return  string
 */
function kek_generate_css_font($class, $var)
{
    $font = '';
    ...
    if (isset($var['font-size']))
        $font .= "font-size: " . kek_px2rem((int)str_replace(' ', 'px', $var['font-size'])) . ";";
 ...
 * @return  string
 */
function kek_import_google_fonts(array $list_fonts, $load = true)
{
    $fonts_url = '';
    $subsets   = $list_variant = $fonts = array();
    $subsets[] = 'latin';
    $subsets[] = 'latin-ext';
    $subset    = _x('no-subset', 'Add new subset (greek, cyrillic, devanagari, vietnamese)', 'sublimeplus');
 ...
 * @return array
 */
function kek_get_font_icons($prefix = '')
{
    $fonts = [
        ...
        'icon' => ['icon-menu','icon-cart','icon-heart','icon-heart-o','icon-refresh','icon-user','icon-search','icon-filter','icon-close','icon-plus','icon-minus','icon-up','icon-down','icon-caret-up','icon-caret-down','icon-arrow-up','icon-arrow-down','icon-arrow-left','icon-arrow-right'],
    ];
    ...
    if ('' === $prefix) {
        ...
        throw new Exception(sprintf(esc_html__('Font with the prefix "%s" not found!', 'sublimeplus'), $prefix));
    }
}
