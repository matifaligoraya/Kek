 ...
 */
if (!function_exists('kek_ajax_get_wishlist_items')) {
    function kek_ajax_get_wishlist_items()
    {
        if (empty($_POST['wishlistItems'])) {
            wp_send_json(['html' => '<p>' . esc_html_e('Wishlist is empty.', 'sublimeplus') . '</p>']);
        }
        ...
        if ($query->have_posts()) : ?>
            <h2 class="wishlist-panel-title popup-panel-title"><?php echo esc_html__('My Wishlist', 'sublimeplus') ?></h2>
                ...
                <table class="wishlist-items-table">
                    ...
                    <tr>
                        ...
                        <th class="product-title" colspan="1">
                            <?php esc_html_e('Product', 'sublimeplus'); ?>
                        </th>
                        <th class="product-price">
                            <?php esc_html_e('Price', 'sublimeplus'); ?>
                        </th>
                        <th class="product-meta">
                            <?php esc_html_e('Product Meta', 'sublimeplus'); ?>
                        </th>
                        <?php if (!kek_enable_catalog_mod()) {
                            ?>
                                ...
                                <div class="content-mobile">
                                    ...
                                    <div class="product-meta">
                                        ...
                                        if ($stock_status == 'instock') {
                                            esc_html_e('In stock', 'sublimeplus');
                                        } else if ($stock_status == 'onbackorder') {
                                            esc_html_e('Back Order', 'sublimeplus');
                                        } else {
                                            esc_html_e('Out of stock', 'sublimeplus');
                                        }
                                        ?>
                                        </span>
                                        <?php
                                    ...
                                    <a href="#" class="remove-from-wishlist"
                                       title="<?php echo esc_attr__('Remove', 'sublimeplus'); ?>"
    ...
    add_action('wp_ajax_kek_get_wishlist_products', 'kek_ajax_get_wishlist_items');
    add_action('wp_ajax_nopriv_kek_get_wishlist_products', 'kek_ajax_get_wishlist_items');
}

/**
 * AJAX get compare products
 */
if (!function_exists('kek_ajax_get_compare_items')) {
    function kek_ajax_get_compare_items()
    {
        if (empty($_POST['compareItems'])) {
            wp_send_json(['html' => '<p>' . esc_html_e('No products to compare.', 'sublimeplus') . '</p>']);
        }
        ...
        if ($query->have_posts()) : ?>
            <h2 class="products-compare-panel-title popup-panel-title">
                <?php echo esc_html__('Compare Products', 'sublimeplus') ?>
            </h2>
            <div class="wrap-popup-content">
                <table class="products-compare-table">
                        ...
                        <?php while ($query->have_posts()) : $query->the_post(); ?>
                            <td class="product-remove products-compare-row-<?php echo esc_attr($post->ID); ?>">
                                <a href="#" class="remove-from-products-compare"
                                   title="<?php echo esc_attr__('Remove', 'sublimeplus'); ?>"
                                   data-id="<?php echo esc_attr($post->ID); ?>"><i class="icon-close"></i></a>
                            </td>
                        <?php endwhile; ?>
                    </tr>
                    <tr>
                    ...
                    <tr>
                        <th><?php esc_html_e('SKU', 'sublimeplus') ?></th>
                        <?php while ($query->have_posts()) : $query->the_post(); ?>
                    ...
                    <tr>
                        <th><?php esc_html_e('Rating', 'sublimeplus') ?></th>
                        <?php while ($query->have_posts()) : $query->the_post(); ?>
                            <td class="product-rating products-compare-row-<?php echo esc_attr($post->ID); ?>">
                                <?php do_action('kek_woo_loop_rating'); ?>
                            </td>
                        <?php endwhile; ?>
                    </tr>
                    <tr>
                        <th><?php esc_html_e('Price', 'sublimeplus') ?></th>
                        <?php while ($query->have_posts()) : $query->the_post(); ?>
                    ...
                    <?php if (!kek_enable_catalog_mod()) { ?>
                        <tr>
                            <th><?php esc_html_e('Add to cart', 'sublimeplus') ?></th>
                            <?php while ($query->have_posts()) : $query->the_post(); ?>
                    ...
                    <?php } ?>
                    <tr>
                        <th><?php esc_html_e('Description', 'sublimeplus') ?></th>
                        <?php while ($query->have_posts()) : $query->the_post(); ?>
                    ...
                    <tr class="attribute-head-group-row">
                        ...
                        ?>
                        <th><?php esc_html_e('Attributes', 'sublimeplus') ?></th>
    ...
    add_action('wp_ajax_kek_get_compare_products', 'kek_ajax_get_compare_items');
    add_action('wp_ajax_nopriv_kek_get_compare_products', 'kek_ajax_get_compare_items');
}
