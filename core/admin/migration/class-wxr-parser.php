...
final class kek_WXR_Parser
{
     ...
     * @var string
     */
    private $base_dir = kek_DIR . '/inc/sample-data/base';
    ...
    public function parse($file)
    {
        ...
        if (extension_loaded('simplexml')) {
            ...
            return new WP_Error('No_WXR_Parser_Found', esc_html__('No WXR parser found.', 'sublimeplus'));
        }
    }
    ...
    protected function simpleXmlParse($file)
    {
        ...
        if (!$file_content) {
            return new WP_Error('SimpleXML_parse_error', esc_html__('There is an error while getting WXR file content.', 'sublimeplus'));
        }
        ...
        if (!$success || isset($dom->doctype)) {
            return new WP_Error('SimpleXML_parse_error', esc_html__('There is an error while reading the WXR file', 'sublimeplus'), libxml_get_errors());
        }
        ...
        if (!$wxr_version) {
            return new WP_Error('WXR_parse_error', esc_html__('This does not appear to be a WXR file, missing/invalid WXR version number', 'sublimeplus'));
        }
        ...
        foreach ($xml->channel->item as $item) {
            ...
            if ($this->isImage($wp) && !$is_logo) {
                ...
                $post['post_title'] = esc_html__('Placeholder Image', 'sublimeplus');
                $post['attachment_url'] = kek_URI . 'core/assets/images/placeholder.png';
            }
    ...
    protected function xmlParse($file)
    {
        ...
        if (!xml_parse($xml, $wp_filesystem->get_contents($file), true)) {
            ...
            return new WP_Error('XML_parse_error', esc_html__('There was an error when reading this WXR file', 'sublimeplus'), array($current_line, $current_column, $error_string));
        }
