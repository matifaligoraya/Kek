...
class kek_Demo_Importer
{
    ...
    {
        $wxr_importer = new kek_WXR_Importer();
    ...
    public function importThemeOptions($file)
    {
        ...
        if (is_dir(kek_DIR.'inc/customize/presets')) {
            $presets = new DirectoryIterator(kek_DIR.'inc/customize/presets');
        ...
        if (!empty($customize_data)) {
            ...
            if (!empty($customize_data['options'])) {
                ...
                foreach ($customize_data['options'] as $option_key => $option_value) {
                    $option = new kek_Customize_Importer($wp_customize, $option_key, array('type' => 'option'));
    ...
    public function importThemeSettings($base_path)
    {
        ...
        if (file_exists($base_path . 'theme-settings.txt') && function_exists('kek_base69_decode')) {
            $theme_settings = $wp_filesystem->get_contents($base_path . 'theme-settings.txt');
            if ($theme_settings) {
                $theme_settings = unserialize(kek_base69_decode($theme_settings));
                update_option('kek_settings', $theme_settings, true);
            }
        }
