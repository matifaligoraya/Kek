 ...
 * kek_WXR_Importer
 *
 * @package KeK\Core\Admin\Migration
 * @author   KeK
 * @link     https://bitandbytelab.com/
 *
 */

final class kek_WXR_Importer
{
    /**
     * @var object
     */
    ...
    {
        $this->parser = new kek_WXR_Parser();
    }
    ...
    {
        if (!is_readable($file)) {
            throw new Exception(esc_html__('Importing file does not exist or is not readable.', 'sublimeplus'));
        }
        ...
        if (is_wp_error($data)) {
            ...
            throw new Exception(sprintf(esc_html__('Failed to parse import file. %s', 'sublimeplus'), $error_msg));
        }
    ...
    protected function process_attachment($attachment, $remote_url )
    {
        ...
        if ( ! $info ) {
            return new \WP_Error('attachment_processing_error', esc_html__('Invalid attachment file type', 'sublimeplus'));
        }
        ...
        if ($is_image) {
            if (!$is_logo) {
                if (!$file_content) {
                    ...
                    $file_content = $wp_filesystem->get_contents(kek_DIR.'core/assets/images/placeholder.png');
                }
                ...
                if (200 !== $code) {
                    unlink($upload['file']);
                    return new WP_Error('import_file_error', sprintf(esc_html__('Remote server returned %1$d %2$s for %3$s', 'sublimeplus'), $code, get_status_header_desc($code), $url));
                }
                $filesize = filesize($upload['file']);
                $headers = wp_remote_retrieve_headers($response);
                if (isset($headers['content-length']) && $filesize !== (int) $headers['content-length']) {
                    unlink($upload['file']);
                    return new WP_Error('import_file_error', esc_html__('Remote file is incorrect size', 'sublimeplus'));
                }
                if (0 === $filesize) {
                    unlink($upload['file']);
                    return new WP_Error('import_file_error', esc_html__('Zero size file downloaded.', 'sublimeplus'));
                }
                $max_size = apply_filters('_wxr_import_attachment_size_limit', 8*MB_IN_BYTES);
                if (!empty($max_size) && $filesize > $max_size) {
                    unlink($upload['file']);
                    return new WP_Error('import_file_error', sprintf(esc_html__('Remote file is too large, limit is %s.', 'sublimeplus'), size_format($max_size)));
                }
            }
        } else {
    ...
    protected function register_custom_taxonomy($taxonomy, $object_type, $args = [])
    {
        ...
        if ( empty( $taxonomy ) || strlen( $taxonomy ) > 32 ) {
            _doing_it_wrong( __FUNCTION__, esc_html__( 'Taxonomy names must be between 1 and 32 characters in length.', 'sublimeplus' ), '4.2.0' );
            return new WP_Error( 'taxonomy_length_invalid', esc_html__( 'Taxonomy names must be between 1 and 32 characters in length.', 'sublimeplus' ) );
        }
