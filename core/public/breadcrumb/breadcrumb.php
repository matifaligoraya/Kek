 ...
 * kek_Breadcrumb
 *
 * @package KeK\Core\Public\Classes
 * @author   KeK
 * @link     https://bitandbytelab.com/
 *
 */
final class kek_Breadcrumb
{
    /**
    ...
    function render(\WP_Query $query)
    {
        ...
        if (!empty($this->separator)) {
            $this->separator = '<span role="presentation" class="breadcrumb-separator"><i class="' . esc_attr($this->separator) . '"></i></span>';
        } else {
            $this->separator = '<span role="presentation" class="breadcrumb-separator"></span>';
        }
        $allow_html = array('span' => array('class' => array(), 'role' => array()), 'i' => array('class' => array()), 'a' => array('href' => array(), 'class' => array(), 'title' => array()));
        ?>
        <div class="breadcrumb-container">
        <span>
                <a class="breadcrumb-url breadcrumb-home" href="<?php echo esc_url(home_url('/')) ?>">
                    <?php
                    if (!$this->home_title) {
                        if ( ! $this->home_icon ) {
    ...
    {
        if(get_the_title( get_option('page_for_posts'))){
            return get_the_title( get_option('page_for_posts'));
        }
        else{
            return esc_html__('Blog', 'sublimeplus');
        }
    ...
    private function theYearCrumb(\WP_Query $query)
    {
        ...
        return $this->getTypeCrumb($query) . esc_html__('Posted in', 'sublimeplus') . ': ' . $y;
    }
    ...
    private function theMonthCrumb(\WP_Query $query)
    {
        ...
        return $this->getCrumbLink(get_year_link($y), $y) . $this->getSep() . esc_html__('Posted on', 'sublimeplus') . ': ' . $m;
    }
    ...
    {
        return esc_html__('Posted by', 'sublimeplus') . ': ' . $query->queried_object->display_name;
    }
    ...
    {
        return esc_html__('Search results for', 'sublimeplus') . ': ' . htmlspecialchars($query->query_vars['s'], ENT_QUOTES);
    }
    ...
    {
        return esc_html__('404 Not Found', 'sublimeplus');
    }
    ...
    {
        return '<a class="breadcrumb-url" href="' . $url . '">' . htmlspecialchars($anchor, ENT_QUOTES) . '</a>';
    }
