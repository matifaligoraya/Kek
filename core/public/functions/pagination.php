 ...
 * Default pagination
 */
if (!function_exists('kek_ajax_pagination'))
{
    function kek_ajax_pagination(WP_Query $query = null, array $args = array())
    {
        ...
        $args = array(
            ...
            'container_selector'   => !empty($args['container_selector']) ? $args['container_selector'] : '.container',
            ...
            'prev_text'            => !empty($args['prev_text']) ? $args['prev_text'] : esc_html__('Prev Posts', 'sublimeplus'),
            'next_text'            => !empty($args['next_text']) ? $args['next_text'] : esc_html__('Next Posts', 'sublimeplus'),
            'horizontal_alignment' => '',
            'gutter'               => '',
            'column_width'         => '',
            'more_text'            => !empty($args['more_text']) ? $args['more_text'] : apply_filters('kek_ajax_pagination_more_text', esc_html__('Load More', 'sublimeplus')),
            'no_more_text'         => !empty($args['no_more_text']) ? $args['no_more_text'] : apply_filters('kek_ajax_pagination_no_more_text', esc_html__('No More Posts', 'sublimeplus')),
        );
        ...
        echo '<div id="spinner-'.esc_attr($uid).'" class="align-center hidden">' . apply_filters('kek_ajax_pagination_spinner', '<img aria-hidden="true" role="presentation" src="'.kek_URI.'core/assets/icons/spinner.svg'.'" width="48" height="48">') . '</div>';

        ?><div id="pagination-<?php echo esc_attr($uid) ?>" class="pagination"><?php
        ...
        $inline_scripts = 'jQuery(document).ready( function($) {
    		...
    		var ias = $.ias({
                ...
                pagination: "#pagination-'.esc_attr($uid).'",
