<?php
/**
 * kek_Customize_Control_Image
 *
 * @package KeK\Core\Customize\Classes\Controls
 *
 */
class kek_Customize_Control_Image extends kek_Customize_Control_Media
{
    /**
     * Print template
     */
    ...
    {
        ?>
        <script type="text/html" id="tmpl-customize-control-image">
        <#
        var required = '';
        ...
        <div class="customize-control customize-control-{{ data.type }} {{ data.class }} customize-control-name-{{ data.original_name }}" data-required="{{ required }}" data-field-name="{{ data.name }}">
            ...
            <div class="customize-control-settings-inner media-type-{{ data.type }}">
                <div class="media">
                    ...
                    <button type="button" class="button add <# if ( url ) { #> hide <# } #>"><?php esc_html_e( 'Select', 'sublimeplus' ); ?></button>
