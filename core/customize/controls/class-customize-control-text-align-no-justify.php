<?php
/**
 * kek_Customize_Control_Text_Align_No_Justify
 *
 * @package KeK\Core\Customize\Classes\Controls
 *
 */
final class kek_Customize_Control_Text_Align_No_Justify extends kek_Customize_Control_Default
{
    /**
     * Print template
     */
    ...
    {
        ?>
        <script type="text/html" id="tmpl-customize-control-text_align_no_justify">
        <#
        var required = '';
        ...
        <div class="customize-control customize-control-{{ data.type }} {{ data.class }} customize-control-name-{{ data.original_name }}" data-required="{{ required }}" data-field-name="{{ data.name }}">
            <#
            if ( _.isUndefined( data.no_justify ) )  {
                    data.no_justify = true;
            }
            data.no_justify = true;
