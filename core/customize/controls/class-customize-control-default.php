<?php
/**
 * kek_Customize_Control_Default
 *
 * @package KeK\Core\Customize\Classes\Controls
 *
 */
class kek_Customize_Control_Default extends WP_Customize_Control
{
    public $type = 'zoo';
    public $choices = [];
    ...
    protected function render()
    {
        ...
        ?><li id="<?php echo esc_attr($id); ?>" class="<?php echo esc_attr($class); ?><?php echo  esc_attr(($this->device) ? ' device-show device-' .$this->device : ''); ?>"><?php
            $this->render_content();
        ?></li><?php
    }
    ...
    protected function render_content()
    {
        ...
        ?><div class="settings-wrapper"><?php
            if ($this->label) :
                ?><div data-control="<?php echo esc_attr($this->id); ?>" class="control-field-header customize-control-heading">
                    <label>
            ...
            if ($this->setting_type != 'custom_html') {
                ?><div class="settings-fields<?php echo esc_attr(($this->setting_type == 'repeater') ? ' repeater-items' : ''); ?>"></div><?php
                if ($this->setting_type == 'repeater') {
                    ?><div class="repeater-actions">
                        <a href="#" class="repeater-reorder" data-text="<?php esc_attr_e('Reorder', 'sublimeplus'); ?>" data-done="<?php esc_attr_e('Done', 'sublimeplus'); ?>"><?php _e('Reorder', 'sublimeplus'); ?></a>
                        <?php if ($this->addable !== false) { ?>
                            <button type="button" class="button repeater-add-new"><?php esc_html_e('Add an item', 'sublimeplus'); ?></button><?php
                        }
                    ?></div><?php
                }
            }
    ...
    {
        ?>
        <script type="text/html" id="tmpl-customize-control-default">
        <#
        var required = '';
        ...
        <# if ( data.label || data.description ) { #>
            ...
            <div class="customize-control-settings-inner">
                <input type="{{ data.type }}" class="input only" data-name="{{ data.name }}" value="{{ data.value }}">
            </div>
        </div>
        </script>
        <?php
