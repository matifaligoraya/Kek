...
class kek_Customize_Live_CSS
{
    ...
    public function setup_shadow($value, $format)
    {
        ...
        $color = $value = kek_sanitize_color_input($p['color']);
    ...
    {
        $value = kek_sanitize_color_input($value);
        if ($format) {
    ...
    {
        $image = kek_customize_get_attachment($value);
    ...
    {
        $values = kek_customize_get_setting($field['name'], 'all');
        ...
        $styling_config = kek_Customizer::get_instance()->get_styling_config();
        $selectorCSSAll = array();
        $selectorCSSDevices = array();
        ...
        foreach (kek_Customizer::SUPPORT_DEVICES as $device) {
            $css = '';
            if (isset($selectorCSSDevices[$device])) {
    ...
    public function maybe_devices_setup($field, $call_back, $values = null, $no_selector = false)
    {
        ...
        if (isset($field['device_settings']) && $field['device_settings']) {
            ...
            foreach (kek_Customizer::SUPPORT_DEVICES as $device) {
                $value = null;
                if (is_null($values)) {
                    $value = kek_customize_get_setting($field['name'], $device);
                } else {
            ...
            if (is_null($values)) {
                $values = kek_customize_get_setting($field['name']);
            }
            if (method_exists($this, $call_back)) {
    ...
    {
        $google_fonts = kek_get_google_fonts();
        $default_fonts = kek_get_websafe_fonts();
    ...
    public function typography($field)
    {
        ...
        $tp_fields = kek_Customizer::get_instance()->get_typo_fields();
    ...
    public function get_font_url()
    {
        ...
        if (null === $url) {
            $url = kek_get_google_fonts_url($this->fonts, $this->variants, $this->subsets);
        }
    ...
    public function auto_css($partial = false)
    {
        ...
        $this->loop_fields(kek_customize_get_all_config());
