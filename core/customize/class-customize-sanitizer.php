 ...
 *
 */
final class kek_Customize_Sanitizer
{
    private $control;
    private $setting;
    private $fonts = [];
                    ...
                    if (is_array($value)) {
                        foreach (kek_Customizer::SUPPORT_DEVICES as $device) {
                            if (isset($value[$device])) {
                                $has_device = true;
                                $value[$device] = kek_sanitize_color_input($value[$device]);
                            }
                        }
                    }
                }
                ...
                if ($device_settings && !$this->skip_devices) {
                    if (is_array($value)) {
                        ...
                        foreach (kek_Customizer::SUPPORT_DEVICES as $device) {
                            if (isset($value[$device])) {
                                $has_device = true;
                                $value[$device] = kek_sanitize_checkbox_value($value[$device]);
                            }
                        }
                    }
                }
                if (!$has_device) {
                    $value = kek_sanitize_checkbox_value($value);
                }
                ...
                if ($device_settings && !$this->skip_devices) {
                    if (is_array($value)) {
                        ...
                        foreach (kek_Customizer::SUPPORT_DEVICES as $device) {
                            if (isset($value[$device])) {
                                $has_device = true;
                                $value[$device] = kek_sanitize_text_input_recursive($value[$device]);
                            }
                        }
                    }
                }
                if (!$has_device) {
                    $value = kek_sanitize_text_input_recursive($value);
                }
