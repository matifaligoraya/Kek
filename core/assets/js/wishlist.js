...
(function(exports, $) {
    ...
    var ZooWishlistView = (function() {
        ...
        Constructor.prototype.renderPanel = function(content) {
            ...
            if (this.panel.length) {
                this.panel.find('.wishlist-panel-inner').html(content);
            } else {
                $('body').append('<div class="wishlist-panel popup-panel" style="display:none"><div class="loading popup-mask-close"></div><button class="wishlist-panel-close-btn popup-close-btn" type="button" name="close-button"><i class="icon-close"></i> </button><div class="wishlist-panel-inner popup-inner" style="display: none">' + content + '</div></div></div>');
            }
        }
        ...
        Constructor.prototype.renderBrowseButton = function(btn) {
            ...
            btn = $(document).find('.wishlist-button[data-id='+btn.attr('data-id')+']');
    ...
    var ZooWishlistController = (function() {
        ...
        function Constructor(model, view) {
            ...
            if (itemsCount < 1) {
                this.view.counter.addClass('hidden');
            } else {
                this.view.counter.removeClass('hidden');
            }
        ...
        Constructor.prototype.sync = function(changeEvent) {
            ...
            if ('zooWishlistItems' === changeEvent.key) {
                ...
                if (items.length < 1) {
                    $('.wishlist-counter').addClass('hidden');
                }
            }
        };
            ...
            if (variationInput.length) {
                variationInput.on('change', function(changeEvent) {
                    var items = self.model.list(),
                        ...
                        btn = $(this).parent().find('.wishlist-button');
            ...
            if (target.hasClass('browse-wishlist') && target.hasClass('redirect-link')) {
                ...
                if (this.model.add(itemID)) {
                    ...
                    if (target.hasClass('redirect-link')) {
                        if ($(document).find('.product-popup-notice')[0]) {
                            $(document).find('.product-popup-notice').addClass('active');
                        } else {
                            $('body').append('<div class="product-popup-notice">' + target.data("label-added") + '</div>');
                            setTimeout(function() {
                                $(document).find('.product-popup-notice').addClass('active');
                            }, 100);
                        }
                        setTimeout(function() {
                            $(document).find('.product-popup-notice').removeClass('active');
                        }, 1200);
                    } else {
                        return this.browseAction(addEvent);
                    }
        ...
        Constructor.prototype.removeAction = function(removeEvent) {
            ...
            var itemID = $(removeEvent.currentTarget).attr('data-id'),
                button = $('.wishlist-button[data-id="' + itemID + '"]'),
        ...
        Constructor.prototype.browseAction = function(browseEvent) {
            ...
            if (items.length && !this.doingAJAX) {
                ...
                $.ajax({
                    ...
                    url: ajaxurl,
                    data: {
                        action: 'sublimeplus_get_wishlist_products',
                    ...
                    self.view.panel.find('.wishlist-panel-inner').fadeIn();
                });
            } else {
                self.view.panel.find('.wishlist-panel-inner').fadeIn();
            }
            $(document).trigger('sublimeplus_browse_wishlist');
        };
        ...
        Constructor.prototype.listen = function() {
            ...
            this.view.panel.find('.wishlist-panel-close-btn').on('click', function(closeEvent) {
