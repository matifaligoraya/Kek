...
(function ($, _) {
	...
	api = zooMegaMenu = {
		init: function () {
			...
			api.itemData = {};
			api.templates = {
				menus     : _.template($('#tmpl-menus').html()),
				title     : _.template($('#tmpl-title').html()),
				mega      : _.template($('#tmpl-mega').html()),
				background: _.template($('#tmpl-background').html()),
				content   : _.template($('#tmpl-content').html())
			};
		...
		initActions: function () {
			...
			api.$body.on('click', '.modal-backdrop, .modal-close, .button-cancel', this.closeModal);
            api.$body.on('change', 'input.enable-mega-menu', function(e)
            {
			...
			api.$modal
				.on('click', '.menu a', this.switchPanel)
				.on('click', '.column-handle', this.resizeMegaColumn)
				.on('click', '.button-save', this.saveChanges);
		},
		...
		closeModal: function () {
			api.$modal.hide().find('.content').html('');
		...
		render: function () {
			// Render menu
			api.$modal.find('.frame-menu .menu').html(api.templates.menus(api.itemData));

			var $activeMenu = api.$modal.find('.menu a.active');
		...
		openSettings: function (panel) {
			var $content = api.$modal.find('.frame-content .content'),
				$panel = $content.children('#panel-' + panel);
			...
			if ($panel.length) {
				...
				$content.children('#panel-' + panel).addClass('active').siblings().removeClass('active');
			...
			// Render title
			var title = api.$modal.find('.frame-menu .menu a[data-panel=' + panel + ']').data('title');
			api.$modal.find('.frame-title').html(api.templates.title({title: title}));
		},
		...
		resizeMegaColumn: function (e) {
			...
			var steps = ['25.00%', '33.33%', '50.00%', '66.66%', '75.00%', '100.00%'],
				$el = $(this),
				$column = $el.closest('.submenu-column'),
		...
		initMegaColumns: function () {
			var $columns = api.$modal.find('#panel-mega .submenu-column'),
				defaultWidth = '25.00%';
		...
		saveChanges: function () {
			var data = $('#menu, .content :input').serialize(),
				$spinner = api.$modal.find('.toolbar .spinner');

			$spinner.addClass('is-active');

			$.post(ajaxurl, {
				action: 'sublimeplus_save_menu_item_data',
				data  : data
			}, function (res) {
				if (!res.success) {
					return;
