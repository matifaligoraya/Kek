(function ($, window, wp) {
  ...
  $(".theme-settings .nav-item").on("click", function (e) {
    e.preventDefault();

    var items, tabs, activeItem, activeTab;

    activeItem = $(this);
    items = $(".theme-settings .nav-item");
    tabs = $(".theme-settings .tab-table");
   ...
   * Handle demo actions
   */
  DOC.on("click", ".theme-setup-site-demo .demo-import", function (e) {
    var btn = $(this),
      demo = btn.data("style"),
    ...
    if ("install" === action && btn.text() === zooAdminL10n.install) {
      ...
      $.post(ajaxurl, {
        action: "sublimeplus_setup_demo_content",
        sublimeplus_demo_slug: demo,
      })
        .done(function (r) {
          var r = JSON.parse(r);
          if (r.success) {
            ...
            currentItem.find(".demo-content").append(
              $(
                '<span class="demo-label">' + zooAdminL10n.installed + "</span>"
              )
                .hide()
                .fadeIn(500)
            );
      ...
      $.post(ajaxurl, {
        action: "sublimeplus_uninstall_demo_content",
        sublimeplus_demo_slug: demo,
      })
        .done(function (r) {
          var r = JSON.parse(r);
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  if (typeof jQuery !== "undefined") {
    alert("test");
    jQuery(".kek-color-field").wpColorPicker({
      defaultColor: "#1db954",
